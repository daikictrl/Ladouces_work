...
travel history, toggle app theme and language, manage notification preferences,
and access the devices section. Profile data comes from Clerk. App settings
(theme, language) are stored locally via Zustand + AsyncStorage.
...
/(tabs)/profile/devices      device connection (see devices.md)
...
Group 2 — App Settings:
  ...
  Notifications     notifications-outline  /(tabs)/profile/settings (scroll to notifications)
...
---                   ---             ---
Trip Reminders        Switch          if off, cancel all trip reminder notif IDs
Packing Reminders     Switch          if off, cancel all checklist notif IDs
Reminder Lead Time    SegmentedControl  "1 day" | "3 hours" | "Both"

Each switch change immediately calls the relevant cancel or reschedule
function from lib/notifications.ts.

LANGUAGE
--------
Setting     Control          Options
---         ---              ---
Language    SelectRow        English | Français

On change: update settingsStore.language.
Language is used for string selection only (no i18n library required at this
stage — use a simple constants/strings.ts file with EN and FR key-value maps).
