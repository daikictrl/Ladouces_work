# MOVANA Smart Reminder & Notification System Implementation Prompt

Implement a complete Smart Travel Reminder and Notification System for the MOVANA mobile application using Expo, React Native, TypeScript, Zustand, NativeWind, and Expo Notifications.

The reminder system must feel modern, intelligent, useful, and deeply integrated into the travel booking workflow.

The feature should help users remember:

* their upcoming trips,
* important travel items,
* and departure times.

The reminder system must become one of the core identity features of MOVANA.

---

# MAIN USER FLOW

After a user successfully books a trip and completes the simulated payment process:

1. Show booking success screen
2. Automatically redirect user to a “Prepare For Your Trip” reminder setup flow
3. Ask the user if they want to create a reminder checklist for the trip
4. Allow the user to:

   * select travel items,
   * add custom reminder items,
   * choose reminder notification times,
   * save the reminder setup
5. Schedule local notifications using Expo Notifications
6. Display reminder progress inside the Home Dashboard and Reminder screens

The flow should feel smooth, premium, and organized.
...
* unnecessary backend notification systems
* overly complex notification architectures

All notifications should be LOCAL scheduled notifications.

---

# REMINDER SETUP SCREEN

Create a modern reminder setup screen immediately after booking.

Screen title:
“Prepare For Your Trip”

Subtitle:
“Let’s help you remember the important things for your journey.”

The screen must contain:

## 1. Suggested Travel Items

Display selectable item chips/cards such as:
...
Selected items should use MOVANA’s blue/purple design language.
...
* type custom reminder items,
* add them dynamically to the checklist.
...
The custom item experience should feel smooth and instant.

---

## 3. Reminder Timing Section

Allow users to choose when notifications should trigger.

Provide smart options:
...
Users should be able to select multiple reminder times.
...
* request notification permission only during reminder setup,
* not request notification permission on app launch,
* gracefully handle denied permissions.

Notification EXAMPLE:

Trip Reminder:
“Your trip to Yaoundé is tomorrow at 6:00 AM.”

Packing Reminder:
“Do not forget your charger and passport.”
...
* work in background state,
* feel timely and useful.

Do not spam users excessively.

---

# REMINDER DATA STRUCTURE

Create a clean reminder architecture linked to bookings.
...
The reminder system must sync cleanly with:
...
Create a dedicated Reminder tab screen accessible from bottom navigation.

The screen should display:

* upcoming reminder trips,
* packing progress,
* checklist completion,
* notification schedules.

Each reminder card should contain:
...
Integrate reminder previews into the Home Dashboard.

Add:

* upcoming trip reminder card,
* countdown timer,
...
Persist reminder data locally.

The reminder system should survive:
...
The reminder system must match MOVANA’s premium design system.
...
* Apple reminders,
* Notion,
* modern travel productivity apps.

---

# IMPORTANT UX RULES

Do NOT force reminder setup.
...
The reminder system should feel helpful, not intrusive.
...
* deeply nested state,
* giant reminder components.

---

# FINAL GOAL

The final reminder system should make MOVANA feel like:

* a smart travel assistant,
* a premium productivity application,
* and an intelligent trip preparation companion.

The experience should emotionally reduce user anxiety about forgetting important travel items before a journey.

# Official Expo Notifications documentation
...
curl -X POST https://api.expo.dev/v2/feedback/docs-send -H 'Content-Type: application/json' -d '{"url":"/versions/v54.0.0/sdk/notifications/","feedback":"🤖 Agent feedback for docs: <specific, actionable description>","model":"<your model identifier>"}'
...
Then proceed to [configuration](/versions/v54.0.0/sdk/notifications#configuration) to set up the [config plugin](/versions/v54.0.0/sdk/notifications#app-config) and obtain the [credentials](/versions/v54.0.0/sdk/notifications#credentials) for push notifications.
...
No usage description is required, see [notification-related permissions](/versions/v54.0.0/sdk/notifications#fetch-information-about-notifications-related-permissions).
...
-   [Configure background notifications](/versions/v54.0.0/sdk/notifications#background-notification-configuration).
-   In your application code, set up a [background task](/versions/v54.0.0/sdk/notifications#registertaskasynctaskname) to run when the notification is received.
...
**If you're using [CNG](/workflow/continuous-native-generation)**, set the [`enableBackgroundRemoteNotifications` property](/versions/v54.0.0/sdk/notifications#configurable-properties) of the config plugin to true, and the correct configuration will be applied automatically by prebuild.
...
After building your app, the array of files will be available for use in both [`NotificationContentInput`](/versions/v54.0.0/sdk/notifications#notificationcontentinput) and [`NotificationChannelInput`](/versions/v54.0.0/sdk/notifications#notificationchannelinput). You only need to provide the base filename. Here's an example using the config above:
...
You can configure the [`notification.icon`](/versions/v54.0.0/config/app#notification) and [`notification.color`](/versions/v54.0.0/config/app#notification) keys in the project's **app.json** if you are using [Expo Prebuild](/workflow/continuous-native-generation) or by using the [`expo-notifications` config plugin directly](/versions/v54.0.0/sdk/notifications#configurable-properties). These are build-time settings, so you'll need to recompile your native Android app with `eas build -p android` or `npx expo run:android` to see the changes.
...
You can also set a custom notification color **per-notification** directly in your [`NotificationContentInput`](/versions/v54.0.0/sdk/notifications#notificationcontentinput) under the `color` attribute.
...
Under the hood, this function is run using `expo-task-manager`. You **must** define the task first, with [`TaskManager.defineTask`](/versions/v54.0.0/sdk/task-manager#taskmanagerdefinetasktaskname-taskexecutor) and register it with `registerTaskAsync`.
...
| interruptionLevel(optional) | `'passive' | 'active' | 'timeSensitive' | 'critical'` | Supported platforms: iOS. The notification’s importance and required delivery timing. Possible values:
